...
The administrator will see a grid that contains all of the users. The grid will have the following columns
...
Unit tests on models and controllers

You can use a grid framework like jqGrid, SlickGrid, DataTables, etc
...
Framework used: jqGrid
  ...
  $ modify Gemfile for database gem (I use Postgres), and modify config/database.yml for database adaptor)
